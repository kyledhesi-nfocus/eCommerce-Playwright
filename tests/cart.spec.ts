...
test.describe('cart tests' , () => {
    for (const coupon of coupons) {
        test(`Coupon Application with '${coupon.code}' @functional`, async ({ populatedCartPage, product }) => {
            ...
            await expect.soft(populatedCartPage.totalPrice, 'Incorrect cart total is displayed - Attaching screenshot to report...').toContainText(await populatedCartPage.calculateExpectedTotal());   // check if the correct total is displayed
    ...
    test.afterEach(async ({ emptyCartPage }) => {
        await expect(emptyCartPage.emptyCart).toBeVisible(); // check if the cart is empty
